<?php
 class Widget_CommentsFeed extends WP_Widget {
 	function Widget_CommentsFeed() 
 	{
		$widget_ops = array('description' => __('Facebook Last Comments Feed', 'fbconnect'));
		$this->WP_Widget('CommentsFeed', __('Facebook Last Comments Feed'), $widget_ops);
 	...
 	function widget($args, $instance) {
 		if (!isset($instance) || count($instance)==0){
			...
			$instance = Widget_CommentsFeed::init_options($instance);
		}
 	...
 	function init_options($options){
		...
		if (!isset($options['welcometext'])){
			$options['welcometext'] = "Last comments ".fb_get_option('blogname')."!";
		}
...
add_action('widgets_init', 'widget_CommentsFeed_init');
function widget_CommentsFeed_init() {
	register_widget('Widget_CommentsFeed');
}
